...
class InternVL2(BaseModel):
    def __init__(self, model, tokenizer, processor=None):
        ...
        self._original_device_map = getattr(model, 'hf_device_map', {})
    ...
    def save_device_map(self, device_map):
        """保存设备映射配置"""
        self._original_device_map = device_map
    ...
    def to_cuda(self):
        ...
        if self._original_device_map is not None:
            # 使用保存的设备映射恢复多卡分配
            self.model = dispatch_model(self.model, device_map=self._original_device_map)
