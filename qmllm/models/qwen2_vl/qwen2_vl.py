...
class Qwen2_VL(BaseModel):  
    ...
    def forward(
        ...
        outputs =  self.model.model(
            ...
            pixel_values=pixel_values.to(next(self.model.parameters()).device) if pixel_values is not None else None,
            pixel_values_videos=pixel_values_videos.to(next(self.model.parameters()).device) if pixel_values_videos is not None else None,
            image_grid_thw=image_grid_thw.to(next(self.model.parameters()).device) if image_grid_thw is not None else None,
            video_grid_thw=video_grid_thw.to(next(self.model.parameters()).device) if video_grid_thw is not None else None,
            position_ids=position_ids.to(next(self.model.parameters()).device) if position_ids is not None else None,
            cache_position=cache_position.to(next(self.model.parameters()).device) if cache_position is not None else None,
            use_cache=use_cache,
    ...
    def to_cuda(self):
        if self.num_params > 10 ** 9: # 1B model  #20 * 10 ** 9
