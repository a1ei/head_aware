...
def auto_scale_block(module, module_kwargs, w_bit, q_config, input_feat, ans_mask, vis_mask, reweight_ratio_dict, loss_mode="mae"):
    ...
    def _search_module_scale(block, linears2scale: list, x, reweight_ratio=None, kwargs={}):
        ...
        for ratio in range(n_grid):
            ...
            if loss_mode == "mse":
                ...
                if ans_mask is not None and vis_mask is not None:
                    ans_mask_expand = ans_mask.unsqueeze(-1).expand_as(out).to(org_out.device)
                    vis_mask_expand = vis_mask.unsqueeze(-1).expand_as(out).cuda().to(org_out.device)
    ...
    elif module.__class__.__name__ == "Qwen2DecoderLayer" or module.__class__.__name__ == "Qwen3VLTextDecoderLayer":
        # attention input
        scales_list.append(
            _auto_get_scale(
                prev_op=module.input_layernorm,
...
def apply_scale(module, scales_list, input_feat_dict=None): 
    for prev_op_name, layer_names, scales in scales_list:
        ...
        elif isinstance(prev_op, (nn.LayerNorm, LlamaRMSNorm)) or prev_op.__class__.__name__ == "InternLM2RMSNorm" or prev_op.__class__.__name__ == "Qwen2RMSNorm" or prev_op.__class__.__name__ == "Qwen3VLTextRMSNorm":
