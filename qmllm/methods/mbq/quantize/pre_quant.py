...
from qmllm.utils.search import append_str_prefix, get_op_name, get_hidden_states
...
from .quantizer import get_module_by_name_suffix , PCASimilarityHook
...
class GradCacheHook:
    ...
    def cache_grad_hook(self, module, inp, out, name):
        ...
        if name not in self.grad_dict:
            ...
            self.grad_dict_all[name] = {"vis_grad": [], "cap_grad": []}
        ...
        for batch_idx in range(B):
            ...
            self.grad_dict_all[name]["vis_grad"].append(vis_grad.abs().detach().cpu())
            self.grad_dict_all[name]["cap_grad"].append(cap_grad.abs().detach().cpu())
    ...
    def get_avg_grad_dict2(self):
        avg_grad_dict = {}

        for name, grad_values in self.grad_dict.items():

            mean_vis = torch.mean(torch.stack(sorted(grad_values["vis_grad"], key=lambda x: x.item(), reverse=True)[2:9]))
            mean_cap = torch.mean(torch.stack(sorted(grad_values["cap_grad"], key=lambda x: x.item(), reverse=True)[50:70]))
...
def get_blocks(model):
    ...
    elif model.__class__.__name__ == "Qwen3VLForConditionalGeneration":
        layers = model.model.language_model.layers
...
def run_mbq(
    ...
    # model.to_cuda()
    # handles = []
    # hooks = PCASimilarityHook(vision_mask, caption_mask, k=10)
    # for layer in model.model.model.language_model.layers:   # 视你的模型结构而定
    #     handles.append(layer.register_forward_hook(hooks.hook_fn))
    # for layer in model.model.model.language_model.layers:   # 视你的模型结构而定
    #     handles.append(layer.register_forward_hook(hooks.hook_fn_for_text_diversity))
    # model(**inputs)
    # print(hooks.layer_sims)
    # print(hooks.layer_sims_text)
    # for h in handles:
    #     h.remove()
    ...
    if reweight:
        ...
        model.to_cpu()
        grad_avg_dict = grad_cache.get_avg_grad_dict() #这里改成2了
