...
#——————————————————————————————————————————————————————————————————————————————————
def get_hidden_states(layer_out):
    # 1) 直接就是 hidden_states: Tensor
    if torch.is_tensor(layer_out):
        return layer_out

    # 2) HF 的 ModelOutput（有 last_hidden_state 或者支持 dict 访问）
    if hasattr(layer_out, "last_hidden_state") and layer_out.last_hidden_state is not None:
        return layer_out.last_hidden_state

    # 3) tuple / list: 约定第 0 个是 hidden_states
    if isinstance(layer_out, (tuple, list)):
        return layer_out[0]

    # 4) 兜底：有些输出可用字典 key
    if isinstance(layer_out, dict) and "last_hidden_state" in layer_out:
        return layer_out["last_hidden_state"]

    raise TypeError(f"Unknown layer output type: {type(layer_out)}")
